...
const AircraftPowerGenerationMRODashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState<number>(new Date().getFullYear() - 1);
  const [activeTab, setActiveTab] = useState<TabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData(config.dataUrl);

  // Once data loads, clamp selectedYear to an available year (current year − 1 if present, else last available).
  useEffect(() => {
...
export default AircraftPowerGenerationMRODashboard;
