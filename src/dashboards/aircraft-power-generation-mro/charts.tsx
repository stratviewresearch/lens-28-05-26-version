        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {marketSize && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">${marketSize.value.toFixed(2)}M</span>
            </div>
          )}
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                ...
                <YAxis yAxisId="left" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}M`} width={70} />
...
export function MarketTrendChart({ data, segments, title, subtitle, showSegments = false, onSegmentClick, useMillions = false }: MarketTrendChartProps) {
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">${entry.value.toFixed(2)}M</span>
            </div>
          ))}
          {payload.length > 1 && (
            <div className="mt-2 border-t border-border pt-2 flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-bold text-foreground">${total.toFixed(2)}M</span>
            </div>
          )}
  ...
  const tableRows = chartData.map((d) => {
    if (showSegments && segments) return [d.year, ...segments.map((s) => `$${(d[s.name] ?? 0).toFixed(2)}M`), `$${d.total.toFixed(2)}M`];
    return [d.year, `$${d.total.toFixed(2)}M`];
  });
            ...
            <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
                  <XAxis dataKey="year" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} interval={Math.ceil(data.length / 8)} />
                  <YAxis stroke="hsl(215, 20%, 55%)" fontSize={12} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(0)}M`} width={70} />
                  <Tooltip content={<CustomTooltip />} />
                  {showSegments && segments ? (
                    <>
                      <Legend content={renderLegend} />
...
export function SegmentPieChart({ data, year, title, onSegmentClick }: SegmentPieChartProps) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          <div className="mt-1 space-y-1 text-sm">
            <p className="text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">${d.value.toFixed(2)}M</span></p>
  ...
  const tableRows = pieData.map((entry) => [entry.name, `$${entry.value.toFixed(2)}M`, `${((entry.value / total) * 100).toFixed(1)}%`]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-2 shadow-lg text-xs">
          ...
          <p className="text-muted-foreground">${item.value.toFixed(2)}M ({percentage}%)</p>
...
export function DistributionDonutsRow({ endUserData, aircraftData, regionData, equipmentData, componentData, year, onDonutClick, onSliceClick, endUserLabel = "End-User Type", equipmentLabel = "Equipment Type", componentLabel = "Component Type" }: DistributionDonutsRowProps) {
  ...
  return (
    <div className={`grid ${gridCols} gap-4`}>
      ...
      {hasAircraft && <MiniDonut data={aircraftData} year={year} title="Platform Type" tabType="aircraft" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.15} />}
    ...
    if (active && payload && payload.length) {
      return (
        <div className="rounded-lg border border-border bg-popover p-3 shadow-lg">
          ...
          <p className="text-sm text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">${payload[0].value.toFixed(2)}M</span></p>
        </div>
      );
    }
    return null;
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{useMillions ? `$${currentValue.toFixed(2)}M` : `$${currentValue.toFixed(2)}M`}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{useMillions ? `$${forecastValue.toFixed(2)}M` : `$${forecastValue.toFixed(2)}M`}</p>
            </motion.div>
            ...
            <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={segmentData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <YAxis stroke="hsl(215, 20%, 55%)" fontSize={12} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}M`} width={useMillions ? 70 : 50} />
                  <Tooltip content={<CustomTooltip />} />
                  ...
                  {(segmentData ?? []).map((item, idx, arr) => {
                    ...
                    return (
                      <tr key={item.year} className="border-b border-border/50 hover:bg-secondary/50">
                        ...
                        <td className="px-3 py-2 text-right font-mono text-foreground">${item.value.toFixed(2)}M</td>
...
export function StackedBarChart({ data, year, title, subtitle, segmentColors, segmentNames, onSegmentClick, useMillions = false }: StackedBarChartProps) {
        ...
        <div className="max-w-[280px] sm:max-w-xs rounded-lg border border-border bg-popover p-3 sm:p-4 shadow-lg">
          ...
          <div className="mt-2 space-y-2 text-xs sm:text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-mono font-medium text-foreground">${hoveredEntry.value?.toFixed(2)}M</span>
            ...
            <div className="mt-1 pt-1 border-t border-border flex items-center gap-2 text-muted-foreground">
              <span>Bar Total:</span>
              <span className="font-mono font-medium text-foreground">${barTotal.toFixed(2)}M</span>
            </div>
          </div>
  ...
  const tableRows = data.map((bar) => [
    bar.name,
    ...segmentNames.map((segName) => { const seg = bar.segments.find((s) => s.name === segName); return `$${(seg?.value ?? 0).toFixed(2)}M`; }),
    `$${bar.total.toFixed(2)}M`,
  ]);
            ...
            <div style={{ height: `${Math.max(200, chartData.length * 50)}px` }} className="w-full -mx-4 sm:mx-0 overflow-x-auto overflow-y-hidden">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 10, left: 20, bottom: 20 }}>
                  ...
                  <XAxis type="number" tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}M`} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} axisLine={{ stroke: "hsl(var(--border))" }} />
          ...
          <tbody className="divide-y divide-border">
            {tableData.map((row, idx) => (
              <motion.tr key={row.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + idx * 0.05 }}
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-muted-foreground whitespace-nowrap">${row.startValue.toFixed(2)}M</td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-foreground whitespace-nowrap">${row.endValue.toFixed(2)}M</td>
