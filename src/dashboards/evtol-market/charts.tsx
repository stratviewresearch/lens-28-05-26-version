...
function formatAxisValue(value: number, useMillions: boolean, isUnits: boolean): string {
  if (isUnits) return value >= 1000 ? `${Math.round(value / 1000)}K` : `${Math.round(value)}`;
  return useMillions ? `$${Math.round(value)}M` : `$${(value / 1000).toFixed(1)}B`;
}
function formatDisplayValue(value: number, useMillions: boolean, isUnits: boolean): string {
  if (isUnits) return Math.round(value).toLocaleString();
  return useMillions ? `$${Math.round(value)}M` : `$${(value / 1000).toFixed(2)}B`;
}
function unitSuffix(isUnits: boolean): string { return isUnits ? " Units" : ""; }
...
export function MarketOverviewChart({ data, title, subtitle, useMillions = false, isUnits = false }: MarketOverviewChartProps) {
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {marketSize && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">{formatDisplayValue(marketSize.value, useMillions, isUnits)}{isUnits ? " Units" : ""}</span>
            </div>
          )}
  ...
  const renderLegend = () => (
    <div className="mt-3 sm:mt-4 flex flex-wrap justify-center gap-3 sm:gap-6">
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <span className="text-xs sm:text-sm text-muted-foreground">Market Size ({isUnits ? "Units" : "US$ M"})</span>
      </div>
      <div className="flex items-center gap-1.5 sm:gap-2">
  ...
  const tableHeaders = ["Year", `Market Size (${isUnits ? "Units" : "US$ M"})`, "YoY Growth (%)"];
  const tableRows = chartData.map((d) => [
    d.year, `${formatDisplayValue(d.value, useMillions, isUnits)}${isUnits ? " Units" : ""}`,
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                ...
                <YAxis yAxisId="left" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} tickFormatter={(value) => formatAxisValue(value, useMillions, isUnits)} width={70} />
...
export function MarketTrendChart({ data, segments, title, subtitle, showSegments = false, onSegmentClick, useMillions = false, isUnits = false }: MarketTrendChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const { downloadChart } = useChartDownload();
  const [view, setView] = useState<"chart" | "table">("chart");
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">{formatDisplayValue(entry.value, useMillions, isUnits)}{isUnits ? " Units" : ""}</span>
            </div>
          ))}
          {payload.length > 1 && (
            <div className="mt-2 border-t border-border pt-2 flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-bold text-foreground">{formatDisplayValue(total, useMillions, isUnits)}{isUnits ? " Units" : ""}</span>
            </div>
          )}
  ...
  const tableRows = chartData.map((d) => {
    if (showSegments && segments) return [d.year, ...segments.map((s) => `${formatDisplayValue((d[s.name] ?? 0), useMillions, isUnits)}${isUnits ? " Units" : ""}`), `${formatDisplayValue(d.total, useMillions, isUnits)}${isUnits ? " Units" : ""}`];
    return [d.year, `${formatDisplayValue(d.total, useMillions, isUnits)}${isUnits ? " Units" : ""}`];
  });
            ...
            <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
                  <XAxis dataKey="year" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} interval={Math.ceil(data.length / 8)} />
                  <YAxis stroke="hsl(215, 20%, 55%)" fontSize={12} tickLine={false} tickFormatter={(value) => formatAxisValue(value, useMillions, isUnits)} width={70} />
                  <Tooltip content={<CustomTooltip />} />
                  {showSegments && segments ? (
                    <>
                      <Legend content={renderLegend} />
...
export function SegmentPieChart({ data, year, title, onSegmentClick }: SegmentPieChartProps) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          <div className="mt-1 space-y-1 text-sm">
            <p className="text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">{d.value.toLocaleString()}</span></p>
  ...
  const tableRows = pieData.map((entry) => [entry.name, entry.value.toLocaleString(), `${((entry.value / total) * 100).toFixed(1)}%`]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-2 shadow-lg text-xs">
          ...
          <p className="text-muted-foreground">{item.value.toLocaleString()} ({percentage}%)</p>
...
export function DrillDownModal({ isOpen, onClose, segmentName, segmentData, color, useMillions = false, isUnits = false }: DrillDownModalProps) {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="rounded-lg border border-border bg-popover p-3 shadow-lg">
          ...
          <p className="text-sm text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">{formatDisplayValue(payload[0].value, useMillions, isUnits)}{isUnits ? " Units" : ""}</span></p>
        </div>
      );
    }
    return null;
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{formatDisplayValue(currentValue, useMillions, isUnits)}{isUnits ? " Units" : ""}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{formatDisplayValue(forecastValue, useMillions, isUnits)}{isUnits ? " Units" : ""}</p>
            </motion.div>
                  ...
                  {(segmentData ?? []).map((item, idx, arr) => {
                    ...
                    return (
                      <tr key={item.year} className="border-b border-border/50 hover:bg-secondary/50">
                        ...
                        <td className="px-3 py-2 text-right font-mono text-foreground">{formatDisplayValue(item.value, useMillions, isUnits)}{isUnits ? " Units" : ""}</td>
...
export function StackedBarChart({ data, year, title, subtitle, segmentColors, segmentNames, onSegmentClick, useMillions = false, isUnits = false }: StackedBarChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const { downloadChart } = useChartDownload();
        ...
        <div className="max-w-[280px] sm:max-w-xs rounded-lg border border-border bg-popover p-3 sm:p-4 shadow-lg">
          ...
          <div className="mt-2 space-y-2 text-xs sm:text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-mono font-medium text-foreground">{formatDisplayValue(hoveredEntry.value ?? 0, useMillions, isUnits)}{isUnits ? " Units" : ""}</span>
            ...
            <div className="mt-1 pt-1 border-t border-border flex items-center gap-2 text-muted-foreground">
              <span>Bar Total:</span>
              <span className="font-mono font-medium text-foreground">{formatDisplayValue(barTotal, useMillions, isUnits)}{isUnits ? " Units" : ""}</span>
            </div>
          </div>
  ...
  const tableRows = data.map((bar) => [
    bar.name,
    ...segmentNames.map((segName) => { const seg = bar.segments.find((s) => s.name === segName); return `${formatDisplayValue((seg?.value ?? 0), useMillions, isUnits)}${isUnits ? " Units" : ""}`; }),
    `${formatDisplayValue(bar.total, useMillions, isUnits)}${isUnits ? " Units" : ""}`,
  ]);
            ...
            <div style={{ height: `${Math.max(200, chartData.length * 50)}px` }} className="w-full -mx-4 sm:mx-0 overflow-x-auto overflow-y-hidden">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 10, left: 20, bottom: 20 }}>
                  ...
                  <XAxis type="number" tickFormatter={(value) => formatAxisValue(value, useMillions, isUnits)} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} axisLine={{ stroke: "hsl(var(--border))" }} />
          ...
          <tbody className="divide-y divide-border">
            {tableData.map((row, idx) => (
              <motion.tr key={row.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + idx * 0.05 }}
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-muted-foreground whitespace-nowrap">{row.startValue.toLocaleString()}</td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-foreground whitespace-nowrap">{row.endValue.toLocaleString()}</td>
