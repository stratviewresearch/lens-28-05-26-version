/**
 * Types, data fetching hook, and utility hooks for Aircraft Soft Goods dashboard.
 * Fetches CSV from remote proxy and parses into MarketData.
 *
 * This dataset has cross-tabulations:
 *   - Aircraft Type (Commercial, Regional, General Aviation, Military, Helicopter) × Region
 *   - Region (NA, Europe, Asia Pacific, RoW) with country breakdowns
 *   - Product Type (Carpet, Seat, Curtains, Others) × Region
 *   - Material Type (Wool/Nylon Blend, Nylon Yarn, Natural Leather, Synthetic Leather, Polyester Fabric, Others) × Region
 *   - End-User Type (OE, Aftermarket) × Region
 */
...
export interface MarketData {
  ...
  productTypeByRegion: Record<string, SegmentData[]>;
...
const COUNTRIES = [
  ...
  "Rest of Europe", "China", "Japan", "India", "Rest of Asia Pacific", "Saudi Arabia", "Brazil",
];
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Soft Goods Market", "Aircraft Soft Goods"]);
  ...
  const productSection = findSection(["By Product Type"]);
  const materialSection = findSection(["By Material Type"]);
  const endUserSection = findSection(["By End-User"]);
  ...
  const productType = (productSection?.topSegments || []).map(ts => toSegmentData(ts));
  const materialType = (materialSection?.topSegments || []).map(ts => toSegmentData(ts));
  const endUser = (endUserSection?.topSegments || []).map(ts => toSegmentData(ts));
  ...
  const productTypeByRegion = buildCrossTabs(productSection);
  const materialTypeByRegion = buildCrossTabs(materialSection);
  const endUserByRegion = buildCrossTabs(endUserSection);

  console.log("[SoftGoods parser] Parsed:", {
    years: years.length,
    ...
    crossTabs: {
      ...
      productTypeByRegion: Object.keys(productTypeByRegion),
      materialTypeByRegion: Object.keys(materialTypeByRegion),
      endUserByRegion: Object.keys(endUserByRegion),
    },
  });

  return {
    years, totalMarket, aircraftType, region, productType, materialType, endUser,
    countryDataByRegion, aircraftTypeByRegion, productTypeByRegion, materialTypeByRegion, endUserByRegion,
  };
}
