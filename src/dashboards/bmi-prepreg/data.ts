/**
 * Types, data fetching hook, and utility hooks for BMI Prepreg dashboard.
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const application = (applicationSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  const furnishedEquipment = (reinforcementSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  ...
  const equipmentByRegion = buildCrossTab(reinforcementSection);
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[BMI Prepreg parser] Sheets found:", sheetNames);
  ...
  console.log("[BMI Prepreg parser] Dual data parsed:", {
