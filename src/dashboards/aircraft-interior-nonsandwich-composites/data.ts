/**
 * Types, data fetching hook, and utility hooks for Aircraft Interior Non-Sandwich Panel Composites dashboard.
...
function parseSectionsFromLines(lines: string[][]): ParsedSection[] {
  ...
  const countryNames = ["USA", "The USA", "Canada", "Mexico", "Germany", "France", "UK", "The UK", "Russia", "Others", "RoE", "Rest of Europe",
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        while (i < lines.length) {
          ...
          if ((isAircraftSection || isApplicationSection || isSalesChannelSection) && currentParent) {
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Interior Non-Sandwich Panel Composites"]);
  ...
  console.log("[Non-Sandwich Panel Composites parser] Parsed:", {
    years: years.length,
