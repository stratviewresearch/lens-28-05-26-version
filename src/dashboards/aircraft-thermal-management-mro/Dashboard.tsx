/**
 * Main Dashboard Page — Aircraft Thermal Management MRO (self-contained).
 */
...
const AircraftThermalManagementMRODashboard = () => {
  ...
  useEffect(() => {
    ...
    if (marketData.years.includes(target)) {
      setSelectedYear(target);
    } else if (!marketData.years.includes(selectedYear)) {
      setSelectedYear(marketData.years[marketData.years.length - 1]);
    }
  ...
  const getSegmentInfo = () => {
    ...
    if (!mapping) return { data: marketData.endUser, title: "Application" };
...
export default AircraftThermalManagementMRODashboard;
