/**
 * Types, data fetching hook, and utility hooks for Aircraft Seats dashboard.
 * Fetches CSV from remote proxy and parses into MarketData.
 *
 * This dataset has cross-tabulations:
 *   - Sales Type (OE, Aftermarket) × Region
 *   - Aircraft Type (NB, WB, Regional, Business Jets) × Region
 *   - Region (NA, Europe, Asia Pacific, RoW) with country breakdowns
 *   - Seat Class Type (First, Business, Premium Economy, Economy) × Region
 *   - Component (Soft Goods, Metals, Composites, Electric) × Region
 *   - Sales Channel Type (BFE, SFE) × Region
 */
...
export interface MarketData {
  ...
  seatClassByRegion: Record<string, SegmentData[]>;
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        // All non-region sections have region children
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Seats Market", "Aircraft Seats"]);
  ...
  const salesTypeSection = findSection(["By Sales Type"]);
  const aircraftSection = findSection(["By Aircraft Type"]);
  const regionSection = findSection(["By Region"]);
  const seatClassSection = findSection(["By Seat Class"]);
  const componentSection = findSection(["By Component"]);
  const salesChannelSection = findSection(["By Sales Channel"]);

  // Flat segments
  const endUser = (salesTypeSection?.topSegments || []).map(ts => toSegmentData(ts));
  ...
  const seatClass = (seatClassSection?.topSegments || []).map(ts => toSegmentData(ts));
  const component = (componentSection?.topSegments || []).map(ts => toSegmentData(ts));
  const furnishedEquipment = (salesChannelSection?.topSegments || []).map(ts => toSegmentData(ts));

  // Cross-tabulations — all have region children
  const endUserByRegion: Record<string, SegmentData[]> = {};
  if (salesTypeSection) {
    salesTypeSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) endUserByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }
  ...
  if (aircraftSection) {
    aircraftSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) aircraftTypeByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }
  ...
  if (regionSection) {
    regionSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) countryDataByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  const seatClassByRegion: Record<string, SegmentData[]> = {};
  if (seatClassSection) {
    seatClassSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) seatClassByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  const componentByRegion: Record<string, SegmentData[]> = {};
  if (componentSection) {
    componentSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) componentByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  const equipmentByRegion: Record<string, SegmentData[]> = {};
  if (salesChannelSection) {
    salesChannelSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) equipmentByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  console.log("[Seats parser] Parsed:", {
    years: years.length,
    endUser: endUser.map(s => s.name),
    aircraftType: aircraftType.map(s => s.name),
    region: region.map(s => s.name),
    seatClass: seatClass.map(s => s.name),
    component: component.map(s => s.name),
    ...
    crossTabs: {
      ...
      seatClassByRegion: Object.keys(seatClassByRegion),
  ...
  return {
    years, totalMarket, endUser, aircraftType, region, seatClass, component, furnishedEquipment,
    countryDataByRegion, endUserByRegion, aircraftTypeByRegion, seatClassByRegion, componentByRegion, equipmentByRegion,
  };
}
