/**
 * Types, data fetching hook, and utility hooks for Galley Inserts MRO dashboard.
 ...
 *   - By Equipment     → furnishedEquipment slot  +  equipmentByRegion
 *   - By Insert Type   → endUser slot             +  endUserByRegion
 */
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isEquipmentSection = titleLower.includes("equipment");
        const isInsertTypeSection = titleLower.includes("insert type");

        const isChildRow = (name: string): boolean => {
          if (isPlatformSection)   return isRegion(name);
          if (isRegionSection)     return isCountry(name);
          if (isEquipmentSection)  return isRegion(name);
          if (isInsertTypeSection) return isRegion(name);
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Galley Inserts MRO", "MRO Market"]);
  ...
  const equipmentSection  = findSection(["By Equipment"]);
  const insertTypeSection = findSection(["By Insert Type"]);
  ...
  const endUser             = toFilteredSegments(insertTypeSection);
  ...
  const endUserByRegion      = buildFilteredCrossTab(insertTypeSection);
  ...
  console.log("[Galley Inserts MRO parser] Parsed:", {
    years: years.length,
    platformType: aircraftType.map(s => s.name),
    equipment: furnishedEquipment.map(s => s.name),
    insertType: endUser.map(s => s.name),
    ...
    crossTabs: {
      ...
      insertTypeByRegion: Object.keys(endUserByRegion),
