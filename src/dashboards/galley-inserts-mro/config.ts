/**
 * DASHBOARD CONFIGURATION — Galley Inserts MRO Market
 *
 * Slot remapping:
 *   - "aircraft"  tab → Platform Type (data: aircraftType, cross-tab: aircraftTypeByRegion)
 *   - "equipment" tab → Equipment     (data: furnishedEquipment, cross-tab: equipmentByRegion)
 *   - "endUser"   tab → Insert Type   (data: endUser, cross-tab: endUserByRegion)
 *   - "region"    tab → Region        (data: region, cross-tab: countryDataByRegion)
 */

import { BarChart3, Wrench, Plane, Globe, Plug } from "lucide-react";

export type TabType = "overview" | "endUser" | "aircraft" | "region" | "equipment" | "component";

export interface TabConfig {
  id: TabType;
  label: string;
  icon: React.ComponentType<{ className?: string }>;
}

export const config = {
  dataUrl: "https://lens.stratviewresearch.com/reactadmin/web/proxy/55",
  title: "Galley Inserts MRO",
  subtitle: "Global Market Research Dashboard",
  defaultYear: new Date().getFullYear() - 1,
  useMillions: true, // values in US$ Million

  footerText: "Galley Inserts MRO Market Research Report",
  footerUnit: "All values in US$ Million unless otherwise specified",

  backPath: "/dataset/aircraft-mro",
  backLabel: "Back to Aircraft MRO",

  tabs: [
    { id: "overview",  label: "Market Overview", icon: BarChart3 },
    { id: "aircraft",  label: "Platform Type",   icon: Plane },
    { id: "equipment", label: "Equipment",       icon: Wrench },
    { id: "endUser",   label: "Insert Type",     icon: Plug },
    { id: "region",    label: "Region",          icon: Globe },
  ] as TabConfig[],

  labels: {
    endUser: "Insert Type",
    equipment: "Equipment",
    application: "",
    processType: "",
    materialType: "",
  },

  segmentMapping: {
    aircraft:  { dataKey: "aircraftType",        title: "Platform Type" },
    equipment: { dataKey: "furnishedEquipment",  title: "Equipment" },
    endUser:   { dataKey: "endUser",             title: "Insert Type" },
    region:    { dataKey: "region",              title: "Region" },
  } as Record<string, { dataKey: string; title: string }>,

  routePath: "/dashboard/galley-inserts-mro",
  catalog: {
    categoryId: "aerospace-defense",
    datasetId: "aircraft-mro",
    dashboardId: "galley-inserts-mro",
    dashboardName: "Galley Inserts MRO",
    purchased: true,
    datasetName: "Aircraft MRO",
  },
} as const;
