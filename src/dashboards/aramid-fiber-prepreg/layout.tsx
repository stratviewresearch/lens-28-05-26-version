/**
 * Layout components: Header, Skeleton, ScrollToTop, Navigation, YearSelector, UnitToggle
...
import type { TabType, TabConfig, UnitMode } from "./config";

export function DashboardHeader({ title = "Market Dashboard", subtitle = "Global Market Research Dashboard" }: { title?: string; subtitle?: string }) {
  return (
...
export function MainNavigation({ value, onChange, selectedYear, onYearChange, showYearSelector = false, tabs, years }: {
  value: TabType; onChange: (value: TabType) => void; selectedYear?: number; onYearChange?: (year: number) => void; showYearSelector?: boolean; tabs: TabConfig[]; years?: number[];
}) {
  return (
...
export function UnitToggle({ value, onChange }: { value: UnitMode; onChange: (mode: UnitMode) => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
      className="flex items-center rounded-lg border border-border bg-secondary/50 p-1 gap-1">
      <button onClick={() => onChange("value")}
        className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs sm:text-sm font-medium transition-all ${value === "value" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
        <DollarSign className="h-3.5 w-3.5" /><span>US$ Million</span>
      </button>
      <button onClick={() => onChange("volume")}
        className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs sm:text-sm font-medium transition-all ${value === "volume" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
        <Weight className="h-3.5 w-3.5" /><span>Million Lbs</span>
      </button>
    </motion.div>
  );
}
