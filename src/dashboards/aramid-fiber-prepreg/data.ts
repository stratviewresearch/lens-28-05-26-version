/**
 * Types, data fetching hook, and utility hooks for Aramid Fiber Prepreg Market dashboard.
 ...
 *     Thermoset Bifurcation: Phenolic, Epoxy (each with region cross-tabs)
 *   - Prepreg Type: Fabric, Unidirectional (each with region cross-tabs)
...
export interface MarketData {
  ...
  furnishedEquipment: SegmentData[]; // unused — no Fiber Type in this dashboard
  ...
  // Inverted cross-tabs: region → segments
  regionByEndUse: Record<string, SegmentData[]>;
  regionByResinType: Record<string, SegmentData[]>;
  regionByThermoplasticBif: Record<string, SegmentData[]>;
  regionByThermosetBif: Record<string, SegmentData[]>;
  regionByPrepregType: Record<string, SegmentData[]>;
  regionByProcessType: Record<string, SegmentData[]>;
}
...
function buildMarketData(lines: string[][]): MarketData {
  ...
  if (resinStartRow >= 0) {
    ...
    while (r < lines.length) {
      ...
      if (REGIONS.includes(label)) { r++; continue; }
      const segName = label.replace(/\s*Prepreg\s*/g, " ").trim();
      materialType.push(toSegmentData(segName, getValues(lines[r])));
      const regionSegs: SegmentData[] = [];
      for (let ri = 1; ri <= 4; ri++) {
        const rLabel = (lines[r + ri]?.[0] ?? "").trim();
        if (REGIONS.includes(rLabel)) regionSegs.push(toSegmentData(rLabel, getValues(lines[r + ri])));
      }
      if (regionSegs.length > 0) materialTypeTopByRegion[segName] = regionSegs;
      r += 1 + regionSegs.length;
    }
  }

  // ── Thermoplastic Bifurcation ─────────────────────────────
  const thermoplasticBifRow = findRow("Thermoplastic Bifurcation", resinStartRow >= 0 ? resinStartRow + 1 : 0);
  let thermoplasticSubResins: SegmentData[] = [];
  let thermoplasticByRegion: Record<string, SegmentData[]> = {};
  if (thermoplasticBifRow >= 0) {
    const result = parseSegmentSection(thermoplasticBifRow + 1, "Thermoplastic Total");
    thermoplasticSubResins = result.segments;
    thermoplasticByRegion = result.byRegion;
  }

  // ── Thermoset Bifurcation ─────────────────────────────────
  const thermosetBifRow = findRow("Thermoset Bifurcation", thermoplasticBifRow >= 0 ? thermoplasticBifRow + 1 : (resinStartRow >= 0 ? resinStartRow + 1 : 0));
  let thermosetSubResins: SegmentData[] = [];
  let thermosetByRegion: Record<string, SegmentData[]> = {};
  if (thermosetBifRow >= 0) {
    const result = parseSegmentSection(thermosetBifRow + 1, "Thermoset Total");
  ...
  // ── Prepreg Type (Fabric, Unidirectional) ─────────────────
  const prepregTypeStartRow = findRow("Prepreg Type", thermosetBifRow >= 0 ? thermosetBifRow + 1 : 0);
  ...
  // ── Process Type ──────────────────────────────────────────
  const processStartRow = findRow("Process Type", regionStartRow >= 0 ? regionStartRow : 0);
  ...
  // ── Invert (parent→[regionSeg]) into (region→[parentSeg]) ──
  const invertByRegion = (map: Record<string, SegmentData[]>): Record<string, SegmentData[]> => {
    const out: Record<string, SegmentData[]> = {};
    for (const [parent, children] of Object.entries(map)) {
      for (const child of children) {
        const region = child.name;
        if (!out[region]) out[region] = [];
        out[region].push({ name: parent, data: child.data });
      }
    }
    return out;
  };

  const regionByEndUse = invertByRegion(endUserByRegion);
  const regionByResinType = invertByRegion(materialTypeTopByRegion);
  const regionByThermoplasticBif = invertByRegion(thermoplasticByRegion);
  const regionByThermosetBif = invertByRegion(thermosetByRegion);
  const regionByPrepregType = invertByRegion(applicationByRegion);
  const regionByProcessType = invertByRegion(processTypeByRegion);

  console.log("[Aramid Fiber Prepreg parser] Parsed:", {
    years: years.length,
  ...
  return {
    ...
    regionByEndUse: filterZeroCrossTab(regionByEndUse),
    regionByResinType: filterZeroCrossTab(regionByResinType),
    regionByThermoplasticBif: filterZeroCrossTab(regionByThermoplasticBif),
    regionByThermosetBif: filterZeroCrossTab(regionByThermosetBif),
    regionByPrepregType: filterZeroCrossTab(regionByPrepregType),
    regionByProcessType: filterZeroCrossTab(regionByProcessType),
  };
}
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[Aramid Fiber Prepreg parser] Sheets found:", sheetNames);
