...
export function DistributionDonutsRow({ endUserData, aircraftData, regionData, applicationData, equipmentData, processTypeData, materialTypeData, year, onDonutClick, onSliceClick, endUserLabel = "End User", equipmentLabel = "Equipment", processTypeLabel = "Process Type", applicationLabel = "Application", materialTypeLabel = "Core Material" }: DistributionDonutsRowProps) {
  ...
  const donutCount = 1 + (hasAircraft ? 1 : 0) + 1 + (hasApplication ? 1 : 0) + (hasEquipment ? 1 : 0) + (hasMaterialType ? 1 : 0) + (hasProcessType ? 1 : 0);
  const gridCols = donutCount >= 7 ? "grid-cols-2 sm:grid-cols-4 lg:grid-cols-7" : donutCount >= 6 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-6" : donutCount === 5 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4";
  ...
  return (
    <div className={`grid ${gridCols} gap-4`}>
      ...
      {hasMaterialType && <MiniDonut data={materialTypeData} year={year} title={materialTypeLabel} tabType="material" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.33} />}
