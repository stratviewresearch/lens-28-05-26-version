/**
 * Types, data fetching hook, and utility hooks for Aircraft Interior Sandwich Panels dashboard.
...
/**
 * This dataset has a unique structure for End-User:
 *   Section 1 ("By End-User Type"): OE / Aftermarket, each with aircraft type sub-segments
 *   Section 2 ("By End-user Type"): OE / Aftermarket, each with region sub-segments
 *
 * We need to capture both sections and build cross-tabs accordingly.
 */
...
function parseSectionsFromLines(lines: string[][]): ParsedSection[] {
  ...
  // Known region/country names for detecting sub-segments
  const regionNames = ["North America", "Europe", "Asia-Pacific", "Rest of the World", "RoW"];
  const countryNames = ["USA", "The USA", "Canada", "Mexico", "Germany", "France", "UK", "The UK", "Russia", "Others",
    "China", "Japan", "India", "RoAP", "Saudi Arabia", "Brazil"];
  // Aircraft type names used as sub-segments in End-User section 1
  const aircraftNames = ["Narrow Body Aircraft", "Wide-Body Aircraft", "Wide Body Aircraft", "Regional Aircraft", "Business Jets"];
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        // Determine what type of sub-segments to expect
        ...
        const isCoreMaterialSection = sectionTitle.toLowerCase().includes("core material");
        const isResinSection = sectionTitle.toLowerCase().includes("resin type");

        // End-user sections: detect by title match
        ...
        while (i < lines.length) {
          ...
          if (isEndUserSection && currentParent) {
            // End-User section 1 has aircraft types as sub-segments
            // End-User section 2 has regions as sub-segments
            if (aircraftNames.includes(segName) || regionNames.includes(segName)) {
              isSubSegment = true;
            }
          }

          if ((isAircraftSection || isApplicationSection || isCoreMaterialSection) && currentParent) {
          ...
          // Resin type has NO sub-segments
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  // Find all sections - note there are TWO end-user sections
  const totalSection = findSection(["Interior Sandwich Panels Market", "Sandwich Panel"]);
  ...
  const coreMaterialSection = findSection(["Core Material"]);
  const resinSection = findSection(["Resin Type"]);

  // Find the two End-User sections
  const endUserSections = sections.filter(s =>
    s.title.toLowerCase().includes("end-user") || s.title.toLowerCase().includes("end user")
  );

  // Section 1: End-User with aircraft type sub-segments
  // Section 2: End-User with region sub-segments
  // Distinguish by checking what sub-segments look like
  let endUserByAircraftSection: ParsedSection | undefined;
  let endUserByRegionSection: ParsedSection | undefined;

  for (const sec of endUserSections) {
    const firstSeg = sec.segments[0];
    if (firstSeg?.subSegments && firstSeg.subSegments.length > 0) {
      const firstSubName = firstSeg.subSegments[0].name;
      if (["North America", "Europe", "Asia-Pacific", "Rest of the World"].includes(firstSubName)) {
        endUserByRegionSection = sec;
      } else {
        endUserByAircraftSection = sec;
      }
    }
  }
  ...
  // Top-level segments (use the first end-user section for the primary endUser array)
  const endUserSource = endUserByAircraftSection || endUserSections[0];
  const endUser = (endUserSource?.segments || []).map(toSegmentData);
  ...
  const materialType = (coreMaterialSection?.segments || []).map(toSegmentData);
  const processType = (resinSection?.segments || []).map(toSegmentData);
    ...
    for (const seg of section?.segments || []) {
      if (seg.subSegments && seg.subSegments.length > 0) {
        result[seg.name] = seg.subSegments.map(sub => ({
          name: normalizeName(sub.name), data: toYearlyData(sub.values),
        }));
      }
    }
    return result;
  ...
  const materialTypeByRegion = buildCrossTab(coreMaterialSection);
  ...
  console.log("[Sandwich Panels parser] Parsed:", {
    years: years.length,
