/**
 * Types, data fetching hook, and utility hooks for Aircraft Lavatory dashboard.
 * Fetches CSV from remote proxy and parses into MarketData.
 *
 * This dataset has cross-tabulations:
 *   - End-User Type (OE, Aftermarket) × Aircraft Type
 *   - Aircraft Type (NB, WB, Regional, Business Jets) × Region
 *   - Region (NA, Europe, Asia Pacific, RoW) with country breakdowns
 *   - Lavatory Type (Standard, Modular, Customized) × Region
 *   - End-User Type × Region
 *   - Furnished Equipment (BFE, SFE) × Region
 */
...
const COUNTRIES = [
  "USA", "Canada", "Mexico", "Germany", "France", "UK", "Russia", "Others",
  "China", "Japan", "India", "Rest of Asia Pacific", "Saudi Arabia", "Brazil",
];
const AIRCRAFT_TYPES = ["Narrow Body Aircraft", "Wide Body Aircraft", "Wide-Body Aircraft", "Regional Aircraft", "Business Jets"];

function isRegion(name: string): boolean {
  return REGIONS.some(r => name.toLowerCase().startsWith(r.toLowerCase()));
}
function isCountry(name: string): boolean {
  return COUNTRIES.some(c => name === c || name.startsWith(c));
}
function isAircraftType(name: string): boolean {
  return AIRCRAFT_TYPES.some(a => name.toLowerCase().startsWith(a.toLowerCase()));
}
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        // Determine expected child type based on section title
        const titleLower = sectionTitle.toLowerCase();
        const isAircraftSection = titleLower.includes("aircraft type");
        const isRegionSection = titleLower.includes("region");
        const isEndUserSection = titleLower.includes("end user");

        // In "By Aircraft Type" section, top segments are aircraft types, children are regions
        // In "By Region" section, top segments are regions, children are countries
        // In "By End User Type" section, top segments are end users, children are aircraft types OR regions
        // In other sections (Lavatory Type, Equipment), top segments are those types, children are regions
        const isChildRow = (name: string): boolean => {
          if (isAircraftSection) return isRegion(name) || isCountry(name);
          if (isRegionSection) return isCountry(name);
          if (isEndUserSection) return isAircraftType(name) || isRegion(name) || isCountry(name);
          // Default: regions/countries are children
        ...
        while (i < lines.length) {
          ...
          // Determine if this is a child row under the current parent
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Lavatory Market", "Aircraft Lavatory"]);
  ...
  // "By End User Type" appears twice: first with aircraft type children, second with region children
  const endUserSections = sections.filter(s => s.title.toLowerCase().includes("end user type"));
  const endUserByAircraftSection = endUserSections.find(s =>
    s.topSegments.some(ts => ts.children.some(c => isAircraftType(c.name)))
  );
  const endUserByRegionSection = endUserSections.find(s =>
    s.topSegments.some(ts => ts.children.some(c => isRegion(c.name)))
  );

  const aircraftSection = findSection(["By Aircraft Type"]);
  const regionSection = findSection(["By Region"]);
  const lavatorySection = findSection(["By Lavatory Type"]);
  ...
  const application = (lavatorySection?.topSegments || []).map(ts => toSegmentData(ts));
  const furnishedEquipment = (equipmentSection?.topSegments || []).map(ts => toSegmentData(ts));

  // Cross-tabulations: End User by Aircraft Type
  const endUserByAircraftType: Record<string, SegmentData[]> = {};
  if (endUserByAircraftSection) {
    endUserByAircraftSection.topSegments.forEach(ts => {
      endUserByAircraftType[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  // Cross-tabulations: End User by Region
  const endUserByRegion: Record<string, SegmentData[]> = {};
  if (endUserByRegionSection) {
    endUserByRegionSection.topSegments.forEach(ts => {
      endUserByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
    });
  }

  // Cross-tabulations: Aircraft Type by Region
  const aircraftTypeByRegion: Record<string, SegmentData[]> = {};
  if (aircraftSection) {
    aircraftSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) {
        aircraftTypeByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
      }
    });
  }

  // Cross-tabulations: Region with country data
  const countryDataByRegion: Record<string, SegmentData[]> = {};
  if (regionSection) {
    regionSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) {
        countryDataByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
      }
    });
  }

  // Cross-tabulations: Lavatory Type by Region
  const applicationByRegion: Record<string, SegmentData[]> = {};
  if (lavatorySection) {
    lavatorySection.topSegments.forEach(ts => {
      if (ts.children.length > 0) {
        applicationByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
      }
    });
  }

  // Cross-tabulations: Furnished Equipment by Region
  const equipmentByRegion: Record<string, SegmentData[]> = {};
  if (equipmentSection) {
    equipmentSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) {
        equipmentByRegion[ts.name] = ts.children.map(c => toSegmentData(c));
      }
    });
  }

  console.log("[Lavatory parser] Parsed:", {
    years: years.length,
