/**
 * Types, data fetching hook, and utility hooks for Aircraft Stowages dashboard.
 ...
 *   - Stowage Type (Overhead Stowage Bin, Other Stowages) × Region
 */
...
export interface MarketData {
  ...
  stowageTypeByRegion: Record<string, SegmentData[]>;
}
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        // Determine child detection logic based on section
        const isChildRow = (name: string): boolean => {
          if (isRegionSection) return isCountry(name);
          // First end-user section has aircraft type children; second has region children
          if (isEndUserSection) {
            // Check if it's an aircraft type or a region
            if (isAircraftType(name)) return true;
            if (isRegion(name)) return true;
            return false;
          }
          // All other sections (Aircraft Type, Equipment, Stowage) have region children
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const stowageSection = findSection(["By Stowage Type"]);
  ...
  // Use the first end-user section for flat segments (OE, Aftermarket)
  ...
  const stowageType = (stowageSection?.topSegments || []).map(ts => toSegmentData(ts));
  ...
  const stowageTypeByRegion = buildCrossTabs(stowageSection);

  console.log("[Stowages parser] Parsed:", {
    years: years.length,
    ...
    stowageType: stowageType.map(s => s.name),
    crossTabs: {
      ...
      stowageTypeByRegion: Object.keys(stowageTypeByRegion),
    },
  });

  return {
    years, totalMarket, endUser, aircraftType, region, furnishedEquipment, stowageType,
    countryDataByRegion, endUserByAircraftType, endUserByRegion, aircraftTypeByRegion,
    equipmentByRegion, stowageTypeByRegion,
  };
}
