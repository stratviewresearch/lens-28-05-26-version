/**
 * Types, data fetching hook, and utility hooks for Aircraft PSU dashboard.
 * Fetches CSV from remote proxy and parses into MarketData.
 *
 * This dataset has cross-tabulations:
 *   - End-User Type (OE, Aftermarket) × Aircraft Type (first occurrence)
 *   - Aircraft Type (NB, WB, Regional, Business Jets) × Region
 *   - Region (NA, Europe, Asia Pacific, RoW) with country breakdowns
 *   - End-user Type × Region (second occurrence)
 *   - Equipment Type (BFE, SFE) × Region
 *   - Component Type (Oxygen System, Structural, Components) × Region
 */
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isChildRow = (name: string): boolean => {
          ...
          // For Equipment Type and Component Type sections — children are regions
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["PSU Market", "Aircraft PSU"]);
  ...
  // "By End-User Type" / "By End-user Type" appears twice:
  // first with aircraft type children, second with region children
  const endUserSections = sections.filter(s => {
    const t = s.title.toLowerCase();
    return (t.includes("end user") || t.includes("end-user")) && t.includes("type");
  });
  ...
  const equipmentSection = findSection(["By Equipment Type"]);
  const componentSection = findSection(["By Component Type"]);

  // Flat segments
  const endUser = (endUserByAircraftSection?.topSegments || []).filter(hasNonZeroData).map(ts => toSegmentData(ts));
  const aircraftType = toFilteredSegments(aircraftSection);
  const region = toFilteredSegments(regionSection);
  const furnishedEquipment = toFilteredSegments(equipmentSection);
  const application = toFilteredSegments(componentSection);

  // Cross-tabulations
  const endUserByAircraftType: Record<string, SegmentData[]> = {};
  if (endUserByAircraftSection) {
    endUserByAircraftSection.topSegments.filter(hasNonZeroData).forEach(ts => {
      const children = ts.children.filter(hasNonZeroData);
      if (children.length > 0) endUserByAircraftType[ts.name] = children.map(c => toSegmentData(c));
    });
  }

  const endUserByRegion: Record<string, SegmentData[]> = {};
  if (endUserByRegionSection) {
    endUserByRegionSection.topSegments.filter(hasNonZeroData).forEach(ts => {
      const children = ts.children.filter(hasNonZeroData);
      if (children.length > 0) endUserByRegion[ts.name] = children.map(c => toSegmentData(c));
    });
  }
  ...
  const aircraftTypeByRegion = buildFilteredCrossTab(aircraftSection);
  const countryDataByRegion = buildFilteredCrossTab(regionSection);
  const applicationByRegion = buildFilteredCrossTab(componentSection);
  const equipmentByRegion = buildFilteredCrossTab(equipmentSection);

  console.log("[PSU parser] Parsed:", {
    years: years.length,
