/**
 * Types, data fetching hook, and utility hooks for Aircraft Civil MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Cross-tabulations:
 *   - MRO Type (Airframe, Engine, Component, Line) × Region        → `endUserByRegion`
 *   - Aircraft Type (NB, WB, Regional Jet, Turboprop) × OEM        → `aircraftTypeByOEM`
 *   - Region (NA, Europe, AP, RoW) with country sub-region children → `countryDataByRegion`
 *
 * `endUser` holds MRO Type segments so existing SegmentDetailTab UI works unchanged.
 */
...
export interface MarketData {
  ...
  region: SegmentData[];
  application: SegmentData[];          // Component Type
  furnishedEquipment: SegmentData[];   // Equipment Type
  ...
  aircraftTypeByOEM: Record<string, SegmentData[]>;
...
const COUNTRIES = [
  "USA", "The USA", "Canada", "Mexico", "Eastern Europe", "Western Europe",
  "China", "India", "Rest of Asia Pacific",
  "Middle East", "Latin America", "Africa",
];
const AIRCRAFT_TYPES = [
  "Narrow Body Aircraft", "Wide Body Aircraft", "Wide-Body Aircraft",
  "Regional Jet", "Regional Aircraft", "Turboprop",
  "Military Aircraft", "General Aviation", "Civil Helicopter", "Business Jets",
  "Singles", "Light Twins", "Mediums", "Super Mediums/ Heavy", "Super Mediums/Heavy",
];
const OEMS = ["Airbus", "Boeing", "Embraer", "Bombardier", "ATR", "Others"];
...
function isAircraftType(name: string): boolean {
  return AIRCRAFT_TYPES.some(a => name.toLowerCase() === a.toLowerCase());
}
function isOEM(name: string): boolean {
  return OEMS.some(o => name.toLowerCase() === o.toLowerCase());
}
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isMROSection = titleLower.includes("mro type");
        const isAircraftSection = !isMROSection && (titleLower.includes("aircraft type") || titleLower === "by type");
        const isRegionSection = !isMROSection && !isAircraftSection && titleLower.includes("region");
        const isEndUserSection = titleLower.includes("end user") || titleLower.includes("end-user");

        const isChildRow = (name: string): boolean => {
          if (isAircraftSection) return isOEM(name) || isRegion(name) || isCountry(name);
          if (isRegionSection) return isCountry(name);
          if (isMROSection) return isRegion(name) || isCountry(name);
          if (isEndUserSection) return isAircraftType(name) || isRegion(name) || isCountry(name);
          return isOEM(name) || isRegion(name) || isCountry(name);
        };
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Civil Helicopter MRO Market", "General Aviation MRO Market", "Aircraft MRO Market", "MRO Market"]);
  ...
  const mroTypeSection = findSection(["By MRO Type"]);
  // "By Type" is GA's aircraft type segmentation; exclude "By MRO Type" by checking substring carefully.
  const aircraftSection = sections.find(s => {
    const t = s.title.toLowerCase();
    return (t.includes("aircraft type") || t === "by type") && !t.includes("mro");
  });
  const regionSection = findSection(["By Region"]);

  // MRO Type → reuse `endUser` slot so existing UI code renders it.
  const endUser = toFilteredSegments(mroTypeSection);
  ...
  // MRO Type × Region — exposed via `endUserByRegion` so SegmentDetailTab renders it
  const endUserByRegion = buildFilteredCrossTab(mroTypeSection);
  const countryDataByRegion = buildFilteredCrossTab(regionSection);

  // Aircraft Type children may be OEMs (civil dataset) and/or regions. Split them.
  const aircraftTypeByOEM: Record<string, SegmentData[]> = {};
  const aircraftTypeByRegion: Record<string, SegmentData[]> = {};
  if (aircraftSection) {
    aircraftSection.topSegments.filter(hasNonZeroData).forEach(ts => {
      const oemChildren = ts.children.filter(c => isOEM(c.name) && hasNonZeroData(c));
      const regionChildren = ts.children.filter(c => (isRegion(c.name) || isCountry(c.name)) && hasNonZeroData(c));
      if (oemChildren.length > 0) aircraftTypeByOEM[ts.name] = oemChildren.map(c => toSegmentData(c));
      if (regionChildren.length > 0) aircraftTypeByRegion[ts.name] = regionChildren.map(c => toSegmentData(c));
    });
  }
  ...
  console.log("[Civil MRO parser] Parsed:", {
    years: years.length,
    mroType: endUser.map(s => s.name),
    aircraftType: aircraftType.map(s => s.name),
    region: region.map(s => s.name),
    crossTabs: {
      mroTypeByRegion: Object.keys(endUserByRegion),
      aircraftTypeByOEM: Object.keys(aircraftTypeByOEM),
