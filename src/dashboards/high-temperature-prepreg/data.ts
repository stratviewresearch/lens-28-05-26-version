/**
 * Types, data fetching hook, and utility hooks for High Temperature Prepreg dashboard.
...
function parseSectionsFromLines(lines: string[][]): ParsedSection[] {
  ...
  const countryNames = [
    ...
    "Germany", "Germany ", "The Netherlands", "France", "France ", "The UK", "The UK ", "Rest of Europe",
    "China", "China ", "Japan", "India", "Rest of Asia Pacific", "Rest of Asia-Pacific",
    "Middle East", "The Middle East", "Latin America", "Others",
    "Russia", "UK", "Spain",
  ];
      ...
      if (potentialYears.length >= 5) {
        ...
        while (i < lines.length) {
          ...
          if (isSubSegment && currentParent) {
            ...
            currentParent.subSegments.push({ name: segName.trim(), values: segValues });
          } else {
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["High Temperature Prepreg Market"]);
  const resinSection = findSection(["Resin Type"]);
  const endUseSection = findSection(["End-Use Industry", "End Use Industry"]);
  const prepregTypeSection = findSection(["Prepreg Type"]);
  const curingSection = findSection(["Curing Process"]);
  const regionSection = findSection(["By Region"]);

  const years = totalSection?.years || resinSection?.years || [];
  ...
  const application = (resinSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  const endUser = (endUseSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  const furnishedEquipment = (prepregTypeSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  ...
  const applicationByRegion = buildCrossTab(resinSection);
  const endUserByRegion = buildCrossTab(endUseSection);
  const equipmentByRegion = buildCrossTab(prepregTypeSection);
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[HT Prepreg parser] Sheets found:", sheetNames);
  ...
  console.log("[HT Prepreg parser] Dual data parsed:", {
