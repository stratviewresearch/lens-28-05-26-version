/**
 * Types, data fetching hook, and utility hooks for Global Aircraft MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Cross-tabulations:
 *   - MRO Type (Airframe, Engine, Component, Line) × Region   → stored in `endUserByRegion`
 *   - Aircraft Type (NB, WB, Regional, Turboprop, Military, GA, Civil Helicopter) × Region → `aircraftTypeByRegion`
 *   - Region (NA, Europe, AP, RoW) with country/sub-region breakdowns → `countryDataByRegion`
 *
 * `endUser` holds MRO Type segments (Airframe/Engine/Component/Line) so existing
 * SegmentDetailTab and DistributionDonutsRow logic works unchanged.
 */
