/**
 * Types, data fetching hook, and utility hooks for Engine MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Source schema (Excel):
 *   - Total
 *   - By Region                  → flat `region`
 *   - By Consumption Type        → 4 types × Region children → `endUser` + `endUserByRegion`
 *   - By Certificate Holder Type → 5 OEMs × Region children  → `furnishedEquipment` + `equipmentByRegion`
 *   - By Engine Model Type       → 10 models × Region children → `aircraftType` + `aircraftTypeByRegion`
 *   - By Platform Type           → 4 platforms × Region children → `application` + `applicationByRegion`
 */
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isChildRow = (name: string): boolean => !isRegionSection && isRegion(name);
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Engine MRO Market", "MRO Market"]);
  ...
  const consumptionSection = findSection(["By Consumption Type"]);
  const certificateSection = findSection(["By Certificate Holder"]);
  const engineModelSection = findSection(["By Engine Model"]);
  const platformSection = findSection(["By Platform Type"]);
  const regionSection = findSection(["By Region"]);

  const endUser = toFilteredSegments(consumptionSection);             // Consumption Type
  const furnishedEquipment = toFilteredSegments(certificateSection);  // Certificate Holder
  const aircraftType = toFilteredSegments(engineModelSection);        // Engine Model
  const application = toFilteredSegments(platformSection);            // Platform Type
  ...
  const endUserByRegion = buildFilteredCrossTab(consumptionSection);
  const equipmentByRegion = buildFilteredCrossTab(certificateSection);
  const aircraftTypeByRegion = buildFilteredCrossTab(engineModelSection);
  const applicationByRegion = buildFilteredCrossTab(platformSection);
  ...
  console.log("[Engine MRO parser] Parsed:", {
    years: years.length,
    consumption: endUser.map(s => s.name),
    certificateHolders: furnishedEquipment.map(s => s.name),
    engineModels: aircraftType.map(s => s.name),
    platforms: application.map(s => s.name),
    regions: region.map(s => s.name),
    crossTabs: {
      consumptionByRegion: Object.keys(endUserByRegion),
      certificateByRegion: Object.keys(equipmentByRegion),
      engineModelByRegion: Object.keys(aircraftTypeByRegion),
      platformByRegion: Object.keys(applicationByRegion),
    },
  });
