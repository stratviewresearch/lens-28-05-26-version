/**
 * All chart components for Aircraft PSU Market dashboard.
...
export function MarketOverviewChart({ data, title, subtitle, useMillions = false }: MarketOverviewChartProps) {
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {marketSize && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">${marketSize.value.toFixed(2)}B</span>
            </div>
          )}
  ...
  const renderLegend = () => (
    <div className="mt-3 sm:mt-4 flex flex-wrap justify-center gap-3 sm:gap-6">
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <span className="text-xs sm:text-sm text-muted-foreground">Market Size (US$ B)</span>
      </div>
      <div className="flex items-center gap-1.5 sm:gap-2">
  ...
  const tableHeaders = ["Year", "Market Size (US$ B)", "YoY Growth (%)"];
  const tableRows = chartData.map((d) => [
    d.year, `$${d.value.toFixed(2)}B`,
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradient-market-size-psu" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis yAxisId="left" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}B`} width={70} />
...
export function MarketTrendChart({ data, segments, title, subtitle, showSegments = false, onSegmentClick, useMillions = false }: MarketTrendChartProps) {
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">${entry.value.toFixed(2)}B</span>
            </div>
          ))}
          {payload.length > 1 && (
            <div className="mt-2 border-t border-border pt-2 flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-bold text-foreground">${total.toFixed(2)}B</span>
            </div>
          )}
  ...
  const tableHeaders = showSegments && segments ? ["Year", ...segments.map((s) => s.name), "Total"] : ["Year", "Market Size (US$ B)"];
  const tableRows = chartData.map((d) => {
    if (showSegments && segments) return [d.year, ...segments.map((s) => `$${(d[s.name] ?? 0).toFixed(2)}B`), `$${d.total.toFixed(2)}B`];
    return [d.year, `$${d.total.toFixed(2)}B`];
  });
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  <defs>
                    {showSegments && segments ? segments.map((seg, idx) => (
                      <linearGradient key={seg.name} id={`gradient-psu-${idx}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      <linearGradient id="gradient-psu-total" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} />
                        <stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} />
                      </linearGradient>
                    )}
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
                  <XAxis dataKey="year" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} interval={Math.ceil(data.length / 8)} />
                  <YAxis stroke="hsl(215, 20%, 55%)" fontSize={12} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(0)}B`} width={70} />
                  <Tooltip content={<CustomTooltip />} />
                  {showSegments && segments ? (
                    <>
                      <Legend content={renderLegend} />
                      {segments.map((seg, idx) => (
                        <Area key={seg.name} type="monotone" dataKey={seg.name} stroke={CHART_COLORS[idx % CHART_COLORS.length]} fill={`url(#gradient-psu-${idx})`} strokeWidth={2} style={{ cursor: "pointer" }} />
                      ))}
                    </>
                  ) : (
                    <Area type="monotone" dataKey="total" stroke="hsl(192, 95%, 55%)" fill="url(#gradient-psu-total)" strokeWidth={2} name="Market Size" />
                  )}
                </AreaChart>
              </ResponsiveContainer>
            </div>
...
export function SegmentPieChart({ data, year, title, onSegmentClick }: SegmentPieChartProps) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          <div className="mt-1 space-y-1 text-sm">
            <p className="text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">${d.value.toFixed(2)}B</span></p>
  ...
  const tableRows = pieData.map((entry) => [entry.name, `$${entry.value.toFixed(2)}B`, `${((entry.value / total) * 100).toFixed(1)}%`]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-2 shadow-lg text-xs">
          ...
          <p className="text-muted-foreground">${item.value.toFixed(2)}B ({percentage}%)</p>
...
export function DistributionDonutsRow({ endUserData, aircraftData, regionData, equipmentData, componentData, year, onDonutClick, onSliceClick, endUserLabel = "End-User Type", equipmentLabel = "Equipment Type", componentLabel = "Component Type" }: DistributionDonutsRowProps) {
  const hasAircraft = aircraftData && aircraftData.length > 0;
  const hasEquipment = equipmentData && equipmentData.length > 0;
  const hasComponent = componentData && componentData.length > 0;
  const donutCount = 1 + (hasAircraft ? 1 : 0) + 1 + (hasEquipment ? 1 : 0) + (hasComponent ? 1 : 0);
  const gridCols = donutCount >= 5 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-2 lg:grid-cols-4";
  ...
  return (
    <div className={`grid ${gridCols} gap-4`}>
      ...
      {hasEquipment && <MiniDonut data={equipmentData} year={year} title={equipmentLabel} tabType="equipment" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.25} />}
      {hasComponent && <MiniDonut data={componentData} year={year} title={componentLabel} tabType="component" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.3} />}
    </div>
  );
}
...
export function DrillDownModal({ isOpen, onClose, segmentName, segmentData, color, useMillions = false }: DrillDownModalProps) {
  ...
  const { downloadChart } = useChartDownload();
  const gradientId = `drillGradient-psu-${segmentName.replace(/[^a-zA-Z0-9]/g, "-")}`;
    ...
    if (active && payload && payload.length) {
      return (
        <div className="rounded-lg border border-border bg-popover p-3 shadow-lg">
          ...
          <p className="text-sm text-muted-foreground">Value: <span className="font-mono font-medium text-foreground">${payload[0].value.toFixed(2)}B</span></p>
        </div>
      );
    }
    return null;
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{useMillions ? `$${currentValue.toFixed(2)}B` : `$${currentValue.toFixed(2)}B`}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-lg border border-border bg-secondary/30 p-4">
              ...
              <p className="text-xl font-bold text-foreground">{useMillions ? `$${forecastValue.toFixed(2)}B` : `$${forecastValue.toFixed(2)}B`}</p>
            </motion.div>
            ...
            <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={segmentData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <YAxis stroke="hsl(215, 20%, 55%)" fontSize={12} tickLine={false} tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}B`} width={useMillions ? 70 : 50} />
                  <Tooltip content={<CustomTooltip />} />
                  ...
                  {(segmentData ?? []).map((item, idx, arr) => {
                    ...
                    return (
                      <tr key={item.year} className="border-b border-border/50 hover:bg-secondary/50">
                        ...
                        <td className="px-3 py-2 text-right font-mono text-foreground">${item.value.toFixed(2)}B</td>
...
export function StackedBarChart({ data, year, title, subtitle, segmentColors, segmentNames, onSegmentClick, useMillions = false }: StackedBarChartProps) {
        ...
        <div className="max-w-[280px] sm:max-w-xs rounded-lg border border-border bg-popover p-3 sm:p-4 shadow-lg">
          ...
          <div className="mt-2 space-y-2 text-xs sm:text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-mono font-medium text-foreground">${hoveredEntry.value?.toFixed(2)}B</span>
            ...
            <div className="mt-1 pt-1 border-t border-border flex items-center gap-2 text-muted-foreground">
              <span>Bar Total:</span>
              <span className="font-mono font-medium text-foreground">${barTotal.toFixed(2)}B</span>
            </div>
          </div>
  ...
  const tableRows = data.map((bar) => [
    bar.name,
    ...segmentNames.map((segName) => { const seg = bar.segments.find((s) => s.name === segName); return `$${(seg?.value ?? 0).toFixed(2)}B`; }),
    `$${bar.total.toFixed(2)}B`,
  ]);
            ...
            <div style={{ height: `${Math.max(200, chartData.length * 50)}px` }} className="w-full -mx-4 sm:mx-0 overflow-x-auto overflow-y-hidden">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 10, left: 20, bottom: 20 }}>
                  ...
                  <XAxis type="number" tickFormatter={(value) => useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}B`} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} axisLine={{ stroke: "hsl(var(--border))" }} />
          ...
          <tbody className="divide-y divide-border">
            {tableData.map((row, idx) => (
              <motion.tr key={row.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + idx * 0.05 }}
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-muted-foreground whitespace-nowrap">${row.startValue.toFixed(2)}B</td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-foreground whitespace-nowrap">${row.endValue.toFixed(2)}B</td>
