/**
 * Types, data fetching hook, and utility hooks for Global Prepreg Market dashboard.
 * Fetches XLS from remote proxy and parses into DualMarketData (value + volume).
 *
 * Data segments:
 *   - End Use Industry: A&D, Wind Energy, Sporting Goods, Automotive, Civil Engineering, Marine, Others
 *     (each with region cross-tabs: NA, Europe, AP, RoW)
 *   - Resin Type: Thermoset, Thermoplastic (top-level)
 *     Thermoset sub-resins: Epoxy, Phenolic, BMI, Cyanate Ester, Other (each with region cross-tabs)
 *     Thermoplastic sub-resins: PPS, PEEK, Other (each with region cross-tabs)
 *   - Fiber Type: Carbon, Glass, Aramid (each with region cross-tabs)
 *   - Form Type: Fabric, Unidirectional (each with region cross-tabs)
...
export interface MarketData {
  ...
  endUser: SegmentData[];            // End Use Industry (7 segments)
  region: SegmentData[];             // 4 top-level regions
  application: SegmentData[];        // Form Type (Fabric, Unidirectional)
  furnishedEquipment: SegmentData[]; // Fiber Type (Carbon, Glass, Aramid)
  processType: SegmentData[];        // Process Type (Autoclave, OoA, Others)
  materialType: SegmentData[];       // Resin Type top-level (Thermoset, Thermoplastic)
  aircraftType: SegmentData[];       // unused placeholder
  countryDataByRegion: Record<string, SegmentData[]>;
  endUserByRegion: Record<string, SegmentData[]>;       // End-use cross-tab by region
  endUserByAircraftType: Record<string, SegmentData[]>; // unused placeholder
  aircraftTypeByRegion: Record<string, SegmentData[]>;  // unused placeholder
  applicationByRegion: Record<string, SegmentData[]>;   // Form by region
  equipmentByRegion: Record<string, SegmentData[]>;     // Fiber by region
  processTypeByRegion: Record<string, SegmentData[]>;   // Process by region
  materialTypeByRegion: Record<string, SegmentData[]>;  // Resin sub-type breakdown (all 8 sub-resins)
  thermosetSubResins: SegmentData[];   // 5 thermoset sub-resins
  thermoplasticSubResins: SegmentData[]; // 3 thermoplastic sub-resins
  thermosetByRegion: Record<string, SegmentData[]>;     // each thermoset sub-resin → region breakdown
  thermoplasticByRegion: Record<string, SegmentData[]>; // each thermoplastic sub-resin → region breakdown
}
...
function buildMarketData(lines: string[][]): MarketData {
  ...
  // Helper: find row index where col-0 contains a keyword
  ...
  // ── End Use Industry (rows 4-39) ──────────────────────────
  const endUseStartRow = findRow("End Use Industry");
  const endUser: SegmentData[] = [];
  const endUserByRegion: Record<string, SegmentData[]> = {};

  if (endUseStartRow >= 0) {
    let r = endUseStartRow + 1;
    while (r < lines.length) {
      ...
      endUser.push(toSegmentData(segName, segValues));

      // Next 4 rows should be regions
      ...
      if (regionSegs.length > 0) {
        endUserByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }

  // ── Resin Type (top-level: Thermoset, Thermoplastic) ──────
  const resinStartRow = findRow("Resin Type", endUseStartRow + 1);
  const materialType: SegmentData[] = [];
  if (resinStartRow >= 0) {
    let r = resinStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total")) break;
      materialType.push(toSegmentData(label.replace(" Prepreg", ""), getValues(lines[r])));
      r++;
    }
  }

  // ── Thermoset Bifurcation ─────────────────────────────────
  const thermosetStartRow = findRow("Thermoset Bifurcation", resinStartRow >= 0 ? resinStartRow + 1 : 0);
  const thermosetSubResins: SegmentData[] = [];
  const thermosetByRegion: Record<string, SegmentData[]> = {};

  if (thermosetStartRow >= 0) {
    let r = thermosetStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total Thermoset")) break;
      if (REGIONS.includes(label)) { r++; continue; }

      const segName = label.replace(" Prepreg", "").replace(" Prepreg ", "");
      const segValues = getValues(lines[r]);
      thermosetSubResins.push(toSegmentData(segName, segValues));
      ...
      if (regionSegs.length > 0) {
        thermosetByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }

  // ── Thermoplastic sub-resins ──────────────────────────────
  const thermosetTotalRow = findRow("Total Thermoset", thermosetStartRow >= 0 ? thermosetStartRow : 0);
  const thermoplasticStartRow = findRow("Thermoplastic Prepreg", thermosetTotalRow >= 0 ? thermosetTotalRow + 1 : 0);
  const thermoplasticSubResins: SegmentData[] = [];
  const thermoplasticByRegion: Record<string, SegmentData[]> = {};

  if (thermoplasticStartRow >= 0) {
    let r = thermoplasticStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total Thermoplastic")) break;
      if (REGIONS.includes(label)) { r++; continue; }

      const segName = label.replace(" Prepreg", "").replace(" Prepregs", "");
      const segValues = getValues(lines[r]);
      thermoplasticSubResins.push(toSegmentData(segName, segValues));
      ...
      if (regionSegs.length > 0) {
        thermoplasticByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }
  ...
  // Group by parent: Thermoset children and Thermoplastic children
  ...
  // ── Fiber Type ────────────────────────────────────────────
  const fiberStartRow = findRow("Fiber Type", thermoplasticStartRow >= 0 ? thermoplasticStartRow : 0);
  const furnishedEquipment: SegmentData[] = [];
  const equipmentByRegion: Record<string, SegmentData[]> = {};

  if (fiberStartRow >= 0) {
    let r = fiberStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total")) break;
      if (REGIONS.includes(label)) { r++; continue; }

      const segName = label.replace(" Prepreg", "");
      const segValues = getValues(lines[r]);
      furnishedEquipment.push(toSegmentData(segName, segValues));
      ...
      if (regionSegs.length > 0) {
        equipmentByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }
  ...
  // ── Form Type ─────────────────────────────────────────────
  const formStartRow = findRow("Form", fiberStartRow >= 0 ? fiberStartRow : 0);
  const application: SegmentData[] = [];
  const applicationByRegion: Record<string, SegmentData[]> = {};

  if (formStartRow >= 0) {
    let r = formStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total")) break;
      if (REGIONS.includes(label)) { r++; continue; }
      ...
      const segName = label.replace(" Prepreg", "");
      const segValues = getValues(lines[r]);
      application.push(toSegmentData(segName, segValues));
      ...
      if (regionSegs.length > 0) {
        applicationByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }
  ...
  // ── Process Type ──────────────────────────────────────────
  const processStartRow = findRow("Process", formStartRow >= 0 ? formStartRow : 0);
  const processType: SegmentData[] = [];
  const processTypeByRegion: Record<string, SegmentData[]> = {};

  if (processStartRow >= 0) {
    let r = processStartRow + 1;
    while (r < lines.length) {
      const label = (lines[r]?.[0] ?? "").trim();
      if (!label || label.startsWith("Total")) break;
      if (REGIONS.includes(label)) { r++; continue; }
      ...
      const segName = label;
      const segValues = getValues(lines[r]);
      processType.push(toSegmentData(segName, segValues));
      ...
      if (regionSegs.length > 0) {
        processTypeByRegion[segName] = regionSegs;
      }
      r += 1 + regionSegs.length;
    }
  }
  ...
  const regionStartRow = findRow("By Region", processStartRow >= 0 ? processStartRow : 0);
  ...
  if (regionStartRow >= 0) {
    ...
    while (r < lines.length) {
      ...
      // Check if this is a top-level region
      if (REGIONS.includes(label)) {
        const regName = label;
        const regValues = getValues(lines[r]);
        region.push(toSegmentData(regName, regValues));

        // Parse country sub-rows
  ...
  console.log("[Global Prepreg Market parser] Parsed:", {
    years: years.length,
  ...
  return {
    ...
    thermosetSubResins,
    thermoplasticSubResins,
    thermosetByRegion,
    thermoplasticByRegion,
  };
}
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[Global Prepreg Market parser] Sheets found:", sheetNames);
