/**
 * Types, data fetching hook, and utility hooks for Actuation MRO dashboard.
 ...
 *   - By Application Type → endUser slot       +  endUserByRegion
 */
...
export interface MarketData {
  ...
  endUser: SegmentData[];               // Application Type
  ...
  aircraftTypeByOEM: Record<string, SegmentData[]>;      // empty (no OEMs)
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isApplicationSection = titleLower.includes("application type");
        ...
        while (i < lines.length) {
          ...
          let segName = segRow[0]?.trim() ?? "";
          if (!segName) { i++; continue; }
          if (segName.startsWith("Total")) { i++; continue; }
          // Normalize verbose Application Type label to "Electric Actuators"
          if (/^electric\b.*\(/i.test(segName) || /electromech/i.test(segName) || /electro-?hydro/i.test(segName)) {
            segName = "Electric Actuators";
          }
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Actuation MRO", "Actuation Market", "MRO Market"]);
  ...
  // Slot remapping
  const aircraftType = toFilteredSegments(platformSection);   // Platform Type → aircraft slot
  const endUser = toFilteredSegments(applicationSection);     // Application Type → endUser slot
  ...
  console.log("[Actuation MRO parser] Parsed:", {
    years: years.length,
    platformType: aircraftType.map(s => s.name),
    applicationType: endUser.map(s => s.name),
