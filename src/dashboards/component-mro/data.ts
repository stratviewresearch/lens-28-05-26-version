/**
 * Types, data fetching hook, and utility hooks for Component MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Source schema (Excel):
 *   - Total
 *   - By Region (NA, Europe, AP, RoW)
 *   - By Component Type (10 components × Region children)  → `endUser` slot + `endUserByRegion`
 *   - By Platform Type  (4 platforms × Region children)    → `aircraftType` slot + `aircraftTypeByRegion`
 */
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        // Region section: all rows are top segments (no children).
        // Component/Platform sections: region rows are children of the preceding component/platform.
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Component MRO Market", "MRO Market"]);
  ...
  // Component Type → reuse `endUser` slot so existing UI code renders it.
  const endUser = toFilteredSegments(componentSection);
  // Platform Type → reuse `aircraftType` slot.
  ...
  const endUserByRegion = buildFilteredCrossTab(componentSection);
  ...
  console.log("[Component MRO parser] Parsed:", {
    years: years.length,
    components: endUser.map(s => s.name),
    platforms: aircraftType.map(s => s.name),
    regions: region.map(s => s.name),
    crossTabs: {
      componentByRegion: Object.keys(endUserByRegion),
