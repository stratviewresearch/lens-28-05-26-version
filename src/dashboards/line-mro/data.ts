/**
 * Types, data fetching hook, and utility hooks for Line MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Source schema (Excel):
 *   - Total
 *   - By Region                    → flat `region`
 *   - By Inspection Type (Check)   → 4 checks × Region children → `endUser` + `endUserByRegion`
 *   - By Inspection Type (Source)  → In-House / Outsourced × Region children → `furnishedEquipment` + `equipmentByRegion`
 *   - By Platform Type             → 4 platforms × Region children → `aircraftType` + `aircraftTypeByRegion`
 */
...
const SOURCING_NAMES = ["in-house", "in house", "outsourced"];
function isSourcingName(name: string): boolean {
  const n = name.toLowerCase().trim();
  return SOURCING_NAMES.some(s => n === s);
}
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Line MRO Market", "MRO Market"]);
  ...
  // Excel re-uses the "By Inspection Type" header for both Check Type and Sourcing Mode.
  // Disambiguate by inspecting the top segment names of each Inspection Type section.
  const inspectionSections = sections.filter(s => s.title.toLowerCase().includes("by inspection type"));
  const sourcingSection = inspectionSections.find(s =>
    s.topSegments.some(ts => isSourcingName(ts.name))
  );
  const checkSection = inspectionSections.find(s => s !== sourcingSection);

  const platformSection = findSection(["By Platform Type"]);
  const regionSection = findSection(["By Region"]);

  const endUser = toFilteredSegments(checkSection);                  // Check Types
  const furnishedEquipment = toFilteredSegments(sourcingSection);    // In-House / Outsourced
  const aircraftType = toFilteredSegments(platformSection);          // Platforms
  ...
  const endUserByRegion = buildFilteredCrossTab(checkSection);
  const equipmentByRegion = buildFilteredCrossTab(sourcingSection);
  ...
  console.log("[Line MRO parser] Parsed:", {
    years: years.length,
    checks: endUser.map(s => s.name),
    sourcing: furnishedEquipment.map(s => s.name),
    platforms: aircraftType.map(s => s.name),
    regions: region.map(s => s.name),
    crossTabs: {
      checkByRegion: Object.keys(endUserByRegion),
      sourcingByRegion: Object.keys(equipmentByRegion),
