/**
 * Types, data fetching hook, and utility hooks for Aircraft Cargo Liner dashboard.
...
function parseSectionsFromLines(lines: string[][]): ParsedSection[] {
  ...
  const aircraftNames = ["Narrow Body Aircraft", "Wide-Body Aircraft", "Wide Body Aircraft", "Regional Aircraft", "Regional Aircaft", "Business Jet", "Business Jets"];
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        while (i < lines.length) {
          ...
          if ((isAircraftSection || isEquipmentSection) && currentParent) {
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Aircraft Cargo Liner Market"]);
  ...
  console.log("[Cargo Liner parser] Parsed:", {
    years: years.length,
