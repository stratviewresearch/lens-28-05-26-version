    ...
    >
      <div className="mb-4 sm:mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <ChartDownloadButton
            onClick={() => downloadChart(chartRef, `market-overview-chart`, `${config.title} — ${title}`)}
          />
        </div>
      </div>
      <AnimatedViewSwitch
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  fontSize={10}
                  tickLine={false}
                  tickFormatter={(value) => (useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(1)}M`)}
                  width={70}
                />
                <YAxis
                  yAxisId="right"
...
export function MarketTrendChart({
  ...
  const tableHeaders =
    showSegments && segments ? ["Year", ...segments.map((s) => s.name), "Total"] : ["Year", "Market Size (US$ M)"];
  const tableRows = chartData.map((d) => {
    if (showSegments && segments)
      return [d.year, ...segments.map((s) => `$${(d[s.name] ?? 0).toFixed(2)}M`), `$${d.total.toFixed(2)}M`];
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: 0.2 }}
      className="rounded-xl border border-border bg-card p-3 sm:p-6 h-full flex flex-col"
    >
        ...
        <div className="flex items-center gap-1">
          ...
          <ChartDownloadButton
            onClick={() =>
              downloadChart(
                chartRef,
                `market-trend-${title.toLowerCase().replace(/\s+/g, "-")}`,
                `${config.title} — ${title}`,
              )
            }
          />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <YAxis
                    ...
                    fontSize={12}
                    tickLine={false}
                    tickFormatter={(value) => (useMillions ? `$${Math.round(value)}M` : `$${value.toFixed(0)}M`)}
                    width={70}
                  />
                  <Tooltip content={<CustomTooltip />} />
                  {showSegments && segments ? (
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          <div className="mt-1 space-y-1 text-sm">
            <p className="text-muted-foreground">
              Value: <span className="font-mono font-medium text-foreground">${d.value.toFixed(2)}M</span>
            </p>
            <p className="text-muted-foreground">
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-2 shadow-lg text-xs">
          ...
          <p className="text-muted-foreground">
            ${item.value.toFixed(2)}M ({percentage}%)
          </p>
...
export function DistributionDonutsRow({
  ...
  const gridCols =
    donutCount >= 5 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-2 lg:grid-cols-4";
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-3 shadow-lg">
          ...
          <p className="text-sm text-muted-foreground">
            Value: <span className="font-mono font-medium text-foreground">${payload[0].value.toFixed(2)}M</span>
          </p>
        </div>
      );
    }
        ...
        <DialogHeader className="pb-4 mb-2 border-b border-[hsl(217,33%,18%)]">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              ...
              <DialogTitle className="text-base sm:text-xl font-bold text-[hsl(210,40%,96%)]">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            >
              ...
              <p className="text-xl font-bold text-foreground">
                {useMillions ? `$${currentValue.toFixed(2)}M` : `$${currentValue.toFixed(2)}M`}
              </p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 10 }}
            ...
            >
              ...
              <p className="text-xl font-bold text-foreground">
                {useMillions ? `$${forecastValue.toFixed(2)}M` : `$${forecastValue.toFixed(2)}M`}
              </p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 10 }}
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <ChartDownloadButton
                onClick={() =>
                  downloadChart(
                    trendChartRef,
                    `${segmentName}-trend`.toLowerCase().replace(/\s+/g, "-"),
                    `${config.title} — ${segmentName} Trend`,
                  )
                }
              />
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={segmentData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  ...
                  <XAxis
                    ...
                    fontSize={10}
                    tickLine={false}
                    interval={Math.ceil(segmentData.length / 8)}
                  />
                  <YAxis
                    stroke="hsl(215, 20%, 55%)"
                    fontSize={12}
                  ...
                  {(segmentData ?? []).map((item, idx, arr) => {
                    ...
                    return (
                      <tr key={item.year} className="border-b border-border/50 hover:bg-secondary/50">
                        ...
                        <td
                          className={`px-3 py-2 text-right font-mono ${change === null ? "text-muted-foreground" : change >= 0 ? "text-chart-4" : "text-destructive"}`}
                        >
...
export function StackedBarChart({
      ...
      return (
        <div className="max-w-[280px] sm:max-w-xs rounded-lg border border-border bg-popover p-3 sm:p-4 shadow-lg">
          <p className="font-semibold text-foreground text-sm sm:text-base break-words">
            {label} - {activeSegment.segmentName} ({year})
          </p>
  ...
  const tableRows = data.map((bar) => [
    ...
    ...segmentNames.map((segName) => {
      ...
      return `$${(seg?.value ?? 0).toFixed(2)}M`;
    }),
    ...
    >
      ...
      <AnimatedViewSwitch
        ...
        chart={
          <>
            <div
              style={{ height: `${Math.max(200, chartData.length * 50)}px` }}
              className="w-full -mx-4 sm:mx-0 overflow-x-auto overflow-y-hidden"
            >
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 10, left: 20, bottom: 20 }}>
                  ...
                  <Tooltip
                    ...
                    wrapperStyle={{ zIndex: 50, maxWidth: "90vw", pointerEvents: "none" }}
                  />
                  {segmentNames.map((segmentName, index) => (
                    <Bar
                      ...
                      onClick={(entry) => handleBarClick(segmentName, index, entry)}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-secondary/30">
              <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">
                Segment
              </th>
              <th className="px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground">
                {startYear}
              </th>
              <th className="px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground">
                {endYear}
              ...
              {onRowClick && (
                <th className="px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground"></th>
              )}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {tableData.map((row, idx) => (
              ...
              >
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-muted-foreground whitespace-nowrap">
                  ${row.startValue.toFixed(2)}M
                </td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-foreground whitespace-nowrap">
                  ${row.endValue.toFixed(2)}M
                </td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right">
                  <span
                    className={`font-mono text-sm font-medium ${row.cagr >= 0 ? "text-chart-4" : "text-destructive"}`}
                  >
                    {row.cagr >= 0 ? "+" : ""}
                    {row.cagr.toFixed(1)}%
                  </span>
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right">
                  <span
                    className={`font-mono text-sm font-medium ${row.growth >= 0 ? "text-chart-4" : "text-destructive"}`}
                  >
                    {row.growth >= 0 ? "+" : ""}
