/**
 * Types, data fetching hook, and utility hooks for Airframe MRO dashboard.
 * Fetches CSV from remote proxy (US$ Billion) and parses into MarketData.
 *
 * Source schema (Excel):
 *   - Total
 *   - By Region          → flat `region`
 *   - By Provider Type   → 3 providers × Region children → `endUser` + `endUserByRegion`
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        while (i < lines.length) {
          ...
          if (isRegionSection) {
            // Region section: regions are parents; everything else is a country child.
            if (isRegion(segName)) {
              currentParent = { name: segName, values: segValues, children: [] };
              section.topSegments.push(currentParent);
            } else if (currentParent) {
              currentParent.children.push({ name: segName, values: segValues });
            }
            i++;
            continue;
          }

          if (currentParent && isRegion(segName)) {
            currentParent.children.push({ name: segName, values: segValues });
            i++;
            continue;
          }
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Landing Gear MRO", "MRO Market"]);
  ...
  const endUser = toFilteredSegments(platformSection);              // Platform Types
  const aircraftType = toFilteredSegments(applicationSection);      // Applications (Main/Nose)
  ...
  const endUserByRegion = buildFilteredCrossTab(platformSection);
  const aircraftTypeByRegion = buildFilteredCrossTab(applicationSection);
  ...
  console.log("[Landing Gear MRO parser] Parsed:", {
    years: years.length,
    platforms: endUser.map(s => s.name),
    applications: aircraftType.map(s => s.name),
    regions: region.map(s => s.name),
    crossTabs: {
      platformByRegion: Object.keys(endUserByRegion),
      applicationByRegion: Object.keys(aircraftTypeByRegion),
      countryByRegion: Object.keys(countryDataByRegion),
    },
  });
