          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradient-market-size-lav" x1="0" y1="0" x2="0" y2="1">
...
export function MarketTrendChart({ data, segments, title, subtitle, showSegments = false, onSegmentClick, useMillions = false }: MarketTrendChartProps) {
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="h-full rounded-xl border border-border bg-card p-3 sm:p-6">
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  <defs>
                    {showSegments && segments ? segments.map((seg, idx) => (
                      ...
                      <linearGradient id="gradient-lav-total" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} />
                        <stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} />
                      </linearGradient>
                    )}
                  ...
                  {showSegments && segments ? (
                    ...
                    <Area type="monotone" dataKey="total" stroke="hsl(192, 95%, 55%)" fill="url(#gradient-lav-total)" strokeWidth={2} name="Market Size" />
                  )}
                </AreaChart>
              </ResponsiveContainer>
            </div>
...
export function SegmentPieChart({ data, year, title, onSegmentClick }: SegmentPieChartProps) {
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.3 }} className="h-full rounded-xl border border-border bg-card p-3 sm:p-6">
      <div className="mb-4 flex items-start justify-between">
        <div>
          <h3 className="text-lg font-semibold text-foreground">{title}</h3>
          <p className="text-sm text-muted-foreground">{year} Distribution</p>
          <p className="mt-1 text-xs italic text-muted-foreground">
            Change the year using the selector above to update this chart.
          </p>
        </div>
        <div className="flex items-center gap-1">
          <ChartTableViewToggle view={view} onViewChange={setView} />
...
export function DistributionDonutsRow({ entries, year, onDonutClick, onSliceClick }: DistributionDonutsRowProps) {
  const shown = entries.filter(e => e.data && e.data.length > 0);
  const donutCount = shown.length;
  const gridCols = donutCount >= 4 ? "grid-cols-2 sm:grid-cols-2 lg:grid-cols-4" : donutCount === 3 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-3" : "grid-cols-2 sm:grid-cols-2";

  return (
    <div className="space-y-3">
      <p className="text-xs italic text-muted-foreground">
        Showing {year} distribution. Change the year using the selector above to update the data displayed below.
      </p>
      <div className={`grid ${gridCols} gap-4`}>
        {shown.map((e, idx) => (
          <MiniDonut key={e.tabType} data={e.data} year={e.yearOverride ?? year} title={e.title} tabType={e.tabType}
            onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.1 + idx * 0.05} />
        ))}
      </div>
    </div>
  );
    ...
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-4 sm:p-6 overflow-hidden">
      <div className="mb-4 flex items-start justify-between">
        <div>
          ...
          {subtitle && <p className="text-sm text-muted-foreground">{subtitle}</p>}
          <p className="mt-1 text-xs italic text-muted-foreground">
            Showing {year}. Change the year using the selector above to update this chart.
          </p>
        </div>
        <div className="flex items-center gap-1">
          <ChartTableViewToggle view={view} onViewChange={setView} />
