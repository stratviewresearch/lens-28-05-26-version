/**
 * Main Dashboard Page — Traffic Marking Solutions Market.
 */
...
import { CompanyShareTab } from "./CompanyShareTab";

const TrafficMarkingDashboard = () => {
  ...
  // Clamp selectedYear into available years
  const effectiveYear = marketData.years.includes(selectedYear)
    ? selectedYear
    : (marketData.years[marketData.years.length - 1] ?? selectedYear);

  const getSegmentInfo = () => {
    const mapping = config.segmentMapping[activeTab];
    if (!mapping) return { data: [], title: "" };
  ...
  const renderTabContent = () => {
    if (activeTab === "overview") {
      return <MarketOverviewTab marketData={marketData} selectedYear={effectiveYear} onNavigateToTab={setActiveTab} />;
    }
    if (activeTab === "company") {
      return <CompanyShareTab marketData={marketData} />;
    }
  ...
  const showYearSelector = activeTab !== "company";
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <MainNavigation value={activeTab} onChange={setActiveTab}
            selectedYear={effectiveYear} onYearChange={setSelectedYear}
            showYearSelector={showYearSelector} tabs={config.tabs} years={marketData.years} />
        </div>
...
export default TrafficMarkingDashboard;
