...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Airframe MRO Market", "MRO Market"]);
  ...
  const providerSection = findSection(["By Provider Type"]);
  const platformSection = findSection(["By Platform Type"]);
  const regionSection = findSection(["By Region"]);

  const endUser = toFilteredSegments(providerSection);              // Provider Types
  ...
  const endUserByRegion = buildFilteredCrossTab(providerSection);
  ...
  console.log("[Airframe MRO parser] Parsed:", {
    years: years.length,
    providers: endUser.map(s => s.name),
    platforms: aircraftType.map(s => s.name),
    regions: region.map(s => s.name),
    crossTabs: {
      providerByRegion: Object.keys(endUserByRegion),
