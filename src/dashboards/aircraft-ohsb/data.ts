/**
 * Types, data fetching hook, and utility hooks for Aircraft OHSB dashboard.
 ...
 *   - Stowage Bin Type (Shelf Bin, Pivot Bin, Translating Bin) × Region
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["OHSB Market", "Aircraft OHSB"]);
  ...
  const stowageBinSection = findSection(["By Stowage Bin Type"]);
  ...
  const application = (stowageBinSection?.topSegments || []).map(ts => toSegmentData(ts));
  ...
  if (stowageBinSection) {
    stowageBinSection.topSegments.forEach(ts => {
      if (ts.children.length > 0) {
  ...
  console.log("[OHSB parser] Parsed:", {
    years: years.length,
