/**
 * Types, data fetching hook, and utility hooks for Global Toilets MRO dashboard.
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Global Toilets MRO", "Safety MRO", "MRO Market"]);
  ...
  const typeSection = findSection(["By Type"]);

  const aircraftType = toFilteredSegments(platformSection);
  const endUser = toFilteredSegments(typeSection);
  ...
  const endUserByRegion = buildFilteredCrossTab(typeSection);
