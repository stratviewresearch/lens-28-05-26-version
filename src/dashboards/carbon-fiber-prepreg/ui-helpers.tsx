...
export function AnimatedCounter({ value, prefix = "", suffix = "", decimals = 1, duration = 1.5, className = "" }: {
...
const iconColors = { primary: "text-primary", accent: "text-accent", "chart-3": "text-chart-3", "chart-4": "text-chart-4" };

export function KPICard({ title, value, prefix = "$", suffix = "B", decimals = 1, change, changeLabel, icon: Icon, delay = 0, accentColor = "primary", onClick }: {
  title: string; value: number; prefix?: string; suffix?: string; decimals?: number; change?: number; changeLabel?: string;
  icon: LucideIcon; delay?: number; accentColor?: "primary" | "accent" | "chart-3" | "chart-4"; onClick?: () => void;
}) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay }}
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-3">
          ...
          {change !== undefined && (
            <div className="flex items-center gap-2">
              <span className={`text-sm font-medium ${change >= 0 ? "text-chart-4" : "text-destructive"}`}>{change >= 0 ? "+" : ""}{change.toFixed(1)}%</span>
