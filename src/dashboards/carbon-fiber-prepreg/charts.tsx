/**
 * All chart components for Carbon Fiber Prepreg Market dashboard.
    ...
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="rounded-xl border border-border bg-card p-3 sm:p-6 h-full flex flex-col">
      <div className="mb-4 sm:mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <ChartDownloadButton onClick={() => downloadChart(chartRef, `cfp-overview`, `${config.title} — ${title}`)} />
        </div>
      </div>
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="cfp-gradient-market-size" x1="0" y1="0" x2="0" y2="1">
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="rounded-xl border border-border bg-card p-3 sm:p-6 h-full flex flex-col">
      <div className="mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          <ChartTableViewToggle view={view} onViewChange={setView} />
          <ChartDownloadButton onClick={() => downloadChart(chartRef, `cfp-trend-${title.toLowerCase().replace(/\s+/g, "-")}`, `${config.title} — ${title}`)} />
        </div>
      </div>
      <AnimatedViewSwitch view={view}
        chart={
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  <defs>
                    {showSegments && segments ? segments.map((seg, idx) => (
                      <linearGradient key={seg.name} id={`cfp-gradient-${idx}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      <linearGradient id="cfp-gradient-total" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} />
                        <stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} />
                      </linearGradient>
                    )}
                  ...
                  {showSegments && segments ? (
                    <>
                      ...
                      {segments.map((seg, idx) => (
                        <Area key={seg.name} type="monotone" dataKey={seg.name} stroke={CHART_COLORS[idx % CHART_COLORS.length]} fill={`url(#cfp-gradient-${idx})`} strokeWidth={2} style={{ cursor: "pointer" }} />
                      ))}
                    </>
                  ) : (
                    <Area type="monotone" dataKey="total" stroke="hsl(192, 95%, 55%)" fill="url(#cfp-gradient-total)" strokeWidth={2} name="Market Size" />
                  )}
                </AreaChart>
              </ResponsiveContainer>
            </div>
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.3 }} className="rounded-xl border border-border bg-card p-3 sm:p-6 h-full flex flex-col">
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          <ChartTableViewToggle view={view} onViewChange={setView} />
          <ChartDownloadButton onClick={() => downloadChart(chartRef, `cfp-${title.toLowerCase().replace(/\s+/g, "-")}-${year}`, `${config.title} — ${title} (${year})`)} />
        </div>
      </div>
      <AnimatedViewSwitch view={view}
        chart={
...
export function DistributionDonutsRow({ endUseData, regionData, resinTypeData, prepregTypeData, processTypeData, year, onDonutClick, onSliceClick, isVolume = false }: DistributionDonutsRowProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      {hasPrepregType && <MiniDonut data={prepregTypeData} year={year} title="Prepreg Type" tabType="prepregType" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.2} isVolume={isVolume} />}
...
export function DrillDownModal({ isOpen, onClose, segmentName, segmentData, color, useMillions = false, isVolume = false }: {
  ...
  const { downloadChart } = useChartDownload();
  const gradientId = `cfp-drillGradient-${segmentName.replace(/[^a-zA-Z0-9]/g, "-")}`;
        ...
        <div className="space-y-6">
          ...
          <motion.div ref={trendChartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-lg border border-border bg-secondary/20 p-4">
            <div className="mb-4 flex items-center justify-between">
              ...
              <ChartDownloadButton onClick={() => downloadChart(trendChartRef, `cfp-${segmentName}-trend`.toLowerCase().replace(/\s+/g, "-"), `${config.title} — ${segmentName} Trend`)} />
            </div>
