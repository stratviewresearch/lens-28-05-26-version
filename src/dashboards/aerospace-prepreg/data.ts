/**
 * Types, data fetching hook, and utility hooks for Aerospace Prepreg dashboard.
 ...
 *   - Resin Type (high-level): Thermoplastic, Thermosets (with region cross-tabs)
 *   - Resin Sub-Type: PPS, PEEK, PEI, Other Thermoplastics, BMI, Cyanate Ester, Polyimide, Epoxy, Phenolic
 *   - Form Type: UD Tape, Fabric (with region cross-tabs)
 *   - Curing Type: Autoclave, Out of Autoclave, Others (with region cross-tabs)
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        while (i < lines.length) {
          ...
          // Detect sub-segments: region names appearing under a parent segment
...
// ── Resin sub-type detection ──────────────────────────────────

const RESIN_SUB_NAMES = [
  "polyphenylene sulfide", "polyether ether ketone", "polyetherimide",
  "other thermoplastics", "bmi", "cyanate ester", "polyimide", "epoxy",
  "phenolic prepreg", "phenolic",
];

const THERMO_PARENT_NAMES = ["thermoplastic", "thermosets", "thermoset"];

function hasThermoParent(section: ParsedSection): boolean {
  return section.segments.some(s =>
    THERMO_PARENT_NAMES.includes(s.name.trim().toLowerCase())
  );
}

function isResinSubTypeSection(section: ParsedSection): boolean {
  // Structural detection: a sub-type section has the Thermoplastic/Thermosets
  // parent rows AND additional sub-resin children (>2 segments total),
  // OR it matches the heuristic of well-known sub-resin names.
  if (hasThermoParent(section) && section.segments.length > 2) return true;
  return section.segments.some(s =>
    RESIN_SUB_NAMES.some(r => s.name.toLowerCase().includes(r))
  ) && section.segments.length > 3;
}
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Aerospace Prepreg Market", "Aerospace Prepregs"]);
  const fiberSection = findSection(["Fiber Type"]);
  const formSection = findSection(["Form Type"]);
  const curingSection = findSection(["Curing Type"]);

  // Handle multiple "By Resin Type" sections
  const resinSections = sections.filter(s => s.title.toLowerCase().includes("resin type"));
  console.log("[Aerospace Prepreg parser] Resin sections:",
    resinSections.map(s => ({ title: s.title, segs: s.segments.map(g => g.name) })));
  let resinHighLevel: ParsedSection | undefined;
  let resinSubType: ParsedSection | undefined;

  for (const sec of resinSections) {
    if (isResinSubTypeSection(sec)) {
      if (!resinSubType) resinSubType = sec;
    } else if (!resinHighLevel) {
      resinHighLevel = sec;
    }
  }
  // Fallback: if only one resin section exists and it has thermo parents,
  // also use it as sub-type (the children are the sub-resins).
  if (!resinSubType && resinSections.length >= 1) {
    const candidate = resinSections.find(hasThermoParent);
    if (candidate) resinSubType = candidate;
  }

  // Bifurcation sections for region cross-tabs on sub-types
  const thermoplasticBifurcation = findSection(["Thermoplastic Resin Type - Bifurcation"]);
  const thermosetBifurcation = findSection(["Thermoset Resin Type - Bifurcation"]);
  ...
  const materialType = (resinHighLevel?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  const application = (formSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);
  const processType = (curingSection?.segments || []).filter(hasNonZeroData).map(toSegmentData);

  // Resin sub-types: section has two parent headers (Thermoplastic, Thermosets)
  // each followed by their child sub-resins. Split children by parent.
  const THERMOPLASTIC_PARENTS = ["thermoplastic"];
  const THERMOSET_PARENTS = ["thermosets", "thermoset"];
  const isParent = (name: string, list: string[]) => {
    const n = name.trim().toLowerCase();
    return list.some(p => n === p);
  };
  const thermoplasticSubResins: SegmentData[] = [];
  const thermosetSubResins: SegmentData[] = [];
  if (resinSubType) {
    let bucket: "tp" | "ts" | null = null;
    for (const seg of resinSubType.segments) {
      if (isParent(seg.name, THERMOPLASTIC_PARENTS)) { bucket = "tp"; continue; }
      if (isParent(seg.name, THERMOSET_PARENTS)) { bucket = "ts"; continue; }
      if (!hasNonZeroData(seg)) continue;
      const sd = toSegmentData(seg);
      if (bucket === "tp") thermoplasticSubResins.push(sd);
      else if (bucket === "ts") thermosetSubResins.push(sd);
    }
  }
  // endUser = all children flat (used by overview/donut), parent headers excluded
  const endUser: SegmentData[] = [...thermosetSubResins, ...thermoplasticSubResins];
  ...
  const materialTypeByRegion = buildCrossTab(resinHighLevel);
  const applicationByRegion = buildCrossTab(formSection);
  const processTypeByRegion = buildCrossTab(curingSection);

  // Resin sub-type by region from bifurcation sections
  const endUserByRegion: Record<string, SegmentData[]> = {};
  for (const bif of [thermoplasticBifurcation, thermosetBifurcation]) {
    if (bif) {
      for (const seg of bif.segments) {
        if (seg.subSegments && seg.subSegments.length > 0 && hasNonZeroData(seg)) {
          endUserByRegion[seg.name] = seg.subSegments.filter(hasNonZeroData).map(sub => ({
            name: sub.name,
            data: toYearlyData(sub.values),
          }));
        }
  ...
  // Country data by region: not available, but region cross-tabs exist within fiber
  const countryDataByRegion: Record<string, SegmentData[]> = {};

  console.log("[Aerospace Prepreg parser] Parsed:", {
    years: years.length,
    fiberType: furnishedEquipment.map(s => s.name),
    resinType: materialType.map(s => s.name),
    resinSubType: endUser.map(s => s.name),
    formType: application.map(s => s.name),
    curingType: processType.map(s => s.name),
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[Aerospace Prepreg parser] Sheets found:", sheetNames);
