/**
 * Types, data fetching hook, and utility hooks for Glass Fiber Prepreg Market dashboard.
 ...
 *   - Fiber Architecture: Fabric, Unidirectional (each with region cross-tabs)
...
export interface MarketData {
  ...
  endUser: SegmentData[];
  region: SegmentData[];
  application: SegmentData[];        // Fiber Architecture (Fabric, Unidirectional)
...
function buildMarketData(lines: string[][]): MarketData {
  ...
  if (resinStartRow >= 0) {
    ...
    while (r < lines.length) {
      ...
      // Skip region sub-rows
      let ri = r + 1;
      while (ri < lines.length && REGIONS.includes((lines[ri]?.[0] ?? "").trim())) ri++;
      r = ri;
    }
  }

  // ── Thermoplastic Bifurcation ─────────────────────────────
  const thermoplasticBifRow = findRow("Thermoplastic Prepreg Bifurcation", resinStartRow >= 0 ? resinStartRow + 1 : 0);
  ...
  const thermosetBifRow = findRow("Thermoset Prepreg Bifurcation", thermoplasticBifRow >= 0 ? thermoplasticBifRow + 1 : (resinStartRow >= 0 ? resinStartRow + 1 : 0));
  ...
  // ── Fiber Architecture (Fabric, Unidirectional) ───────────
  const prepregTypeStartRow = findRow("Fiber Architecture", thermosetBifRow >= 0 ? thermosetBifRow + 1 : 0);
  ...
  // ── Process Type ──────────────────────────────────────────
  const processStartRow = findRow("Process Type", prepregTypeStartRow >= 0 ? prepregTypeStartRow + 1 : 0);
  ...
  console.log("[Glass Fiber Prepreg parser] Parsed:", {
    years: years.length,
...
function parseWorkbook(buffer: ArrayBuffer): DualMarketData {
  ...
  console.log("[Glass Fiber Prepreg parser] Sheets found:", sheetNames);
