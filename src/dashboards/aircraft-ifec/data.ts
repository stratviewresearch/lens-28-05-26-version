/**
 * Types, data fetching hook, and utility hooks for Aircraft IFEC dashboard.
  ...
  while (i < lines.length) {
    ...
    if (row.length > 2 && row[0] && !row[0].startsWith("(") && !row[0].startsWith("Total")) {
      ...
      if (potentialYears.length >= 5) {
        ...
        const isProductSection = sectionTitle.toLowerCase().includes("product type");
        const isEquipmentSection = sectionTitle.toLowerCase().includes("equipment type") || sectionTitle.toLowerCase().includes("furnished");
        ...
        while (i < lines.length) {
          ...
          if ((isAircraftSection || isProductSection || isEquipmentSection) && currentParent) {
...
function buildMarketData(sections: ParsedSection[]): MarketData {
  ...
  const totalSection = findSection(["Aircraft IFEC Market"]);

  // Two End-User sections: 1st with aircraft type subs, 2nd with region subs
  ...
  const productSection = findSection(["Product Type"]);
  ...
  const application = (productSection?.segments || []).map(toSegmentData);
  ...
  const applicationByRegion = buildCrossTab(productSection);
  const equipmentByRegion = buildCrossTab(equipmentSection);

  console.log("[Aircraft IFEC parser] Parsed:", {
    years: years.length,
    endUser: endUser.length,
    aircraftType: aircraftType.length,
    region: region.length,
    productType: application.length,
